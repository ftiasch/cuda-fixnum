 ...
 */
template <int BYTES_, typename digit_ = u32_fixnum> class warp_fixnum {
public:
  ...
  static_assert(BYTES > 0, "Fixnum bytes must be positive.");
...
public:
  __device__ __forceinline__ warp_fixnum() {}

  // TODO: Shouldn't this be equivalent to the digit_to_fixnum() function
  // below?
  __device__ __forceinline__ warp_fixnum(digit z) : x(z) {}
   ...
   * in.
   */
  __host__ __device__ static int from_bytes(uint8_t *r, const uint8_t *bytes,
                                            int nbytes) {
   ...
   * in.
   */
  __host__ __device__ static int to_bytes(uint8_t *bytes, int nbytes,
                                          const uint8_t *r) {
    int n = min(nbytes, BYTES);
    memcpy(bytes, r, n);
    return n;
  ...
  __device__ static fixnum zero() { return digit::zero(); }

  __device__ static fixnum one() { return digit(layout::laneIdx() == 0); }

  __device__ static fixnum two() {
    return digit(layout::laneIdx() == 0 ? 2 : 0);
  }

  __device__ static int is_zero(fixnum a) { return nonzero_mask(a) == 0; }
  ...
  __device__ static void neg(fixnum &r, fixnum a) { sub(r, zero(), a); }
   ...
   */
  __device__ static void sqr_wide_(fixnum &ss, fixnum &rr, fixnum a) {
    ...
    for (int i = 0; i < W / 2; ++i) {
      ...
      // would be clearer, but the shuffles would (presumably) be more
      // expensive.
  ...
  __device__ __forceinline__ static void sqr_wide(fixnum &ss, fixnum &rr,
                                                  fixnum a) {
  ...
  __device__ static void lshift_small(fixnum &y, fixnum &overflow, fixnum x,
                                      int b) {
    assert(b >= 0);
    assert(b <= digit::BITS);
    int L = layout::laneIdx();
  ...
  __device__ static void lshift_small(fixnum &y, fixnum x, int b) {
    assert(b >= 0);
    assert(b <= digit::BITS);
   ...
   */
  __device__ static void lshift(fixnum &y, fixnum &overflow, fixnum x, int b) {
    assert(b >= 0);
    assert(b <= BITS);
  ...
  __device__ static void lshift(fixnum &y, fixnum x, int b) {
    assert(b >= 0);
    assert(b <= BITS);
    int q = b / digit::BITS, r = b % digit::BITS;
   ...
   * mul_2exp.
   */
  __device__ static void rshift(fixnum &y, fixnum &underflow, fixnum x, int b) {
    lshift(underflow, y, x, BITS - b);
  }

  __device__ static void rshift(fixnum &y, fixnum x, int b) {
    fixnum underflow;
    rshift(y, underflow, x, b);
  }

private:
  __device__ static void digit_to_fixnum(digit &c) {
    int L = layout::laneIdx();
    // TODO: Try without branching?  c &= -(digit)(L == 0);
    c = (L == 0) ? c : digit::zero();
  }

  __device__ static digit effective_carries(digit &cy_hi, int propagate,
                                            int cy) {
    int L = layout::laneIdx();
  ...
  __device__ static digit effective_carries(digit &cy_hi, int propagate, int cy,
                                            int cy_lo) {
